package com.hzncc.kevin.circleselector;
...
public class CircleSeekBar extends View {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (isCanTouch && (event.getAction() == MotionEvent.ACTION_MOVE || isTouch(x, y))) {
            ...
            if (mChangListener != null && (event.getAction() & (MotionEvent.ACTION_MOVE | MotionEvent.ACTION_UP)) > 0) {
                mChangListener.onChanged(this, isFloat ? mCurProcess : (int) mCurProcess);
            }
            invalidate();
            return true;
        } else {
    ...
    private float getSelectedValue() {
        ...
        return b.setScale(floatNum, BigDecimal.ROUND_HALF_UP).floatValue();
    }
